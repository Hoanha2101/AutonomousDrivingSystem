...
def driving_area_mask(seg = None):
    ...
    da_predict = seg[:, :, 12:372,:] # torch.Size([1, 2, 360, 640])
    da_seg_mask = torch.nn.functional.interpolate(da_predict, scale_factor=2, mode='bilinear') # torch.Size([1, 2, 720, 1280])
    _, da_seg_mask = torch.max(da_seg_mask, 1) # torch.Size([1, 720, 1280])
    da_seg_mask = da_seg_mask.int().squeeze().cpu().numpy()#(720, 1280)
    return da_seg_mask
